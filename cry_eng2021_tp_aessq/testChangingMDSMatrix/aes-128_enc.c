 ...
 * Function used to compute p ** 2 over the group F_2[X]/X^8 + X^4 + X^3 + X + 1
 */
 ...
 * Function used to compute p ** 2 over the group F_2[X]/X^8 + X^6 + X^5 + X^4 + X^3 + X + 1
 */
...
void aes_round(uint8_t block[AES_BLOCK_SIZE], uint8_t round_key[AES_BLOCK_SIZE], int lastround)
{
	...
	for (i = lastround; i < 16; i += 4) /* lastround = 16 if it is the last round, 0 otherwise */
	{
		...
		column[3] ^= tmp ^ xtime(column[0] ^ column[1]);
		column[2] ^= tmp ^ xtime(column[1] ^ column[2]);
		column[1] ^= tmp ^ xtime(column[2] ^ column[3]);
		column[0] ^= tmp ^ xtime(column[3] ^ tmp2);
	}
